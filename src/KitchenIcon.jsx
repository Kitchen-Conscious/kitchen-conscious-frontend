...
function KitchenIcon(props) {
  ...
  const deleteKitchen = async (event) => {
    ...
    const response = await fetch(
      `http://localhost:8080/kitchens/${props.kitchenId}`,
      {
        credentials: "include",
        method: "DELETE",
        headers: {
  ...
  return (
    <div>
      <Link href={`../kitchens/${props.kitchenId}`}>

        <div
          className=" flex justify-center align-middle flex flex-col place-content-center w-44 l-80 shadow-black-lg mx-10 cursor-pointer bg-neutral-100 text-black  py-2 hover:bg-green-500/75 rounded-xl font-medium lg:text-xl px:60 pr-4"
        >
          ...
          <button
            ...
            <button
              ...
              type="button"
              className=" ml-3 flex justify-center align-middle content-center text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white">
              <svg
                width="21"
                height="24"
                viewBox="0 0 21 24"
